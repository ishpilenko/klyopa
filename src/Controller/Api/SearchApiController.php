...
class SearchApiController extends AbstractController
{
    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        // SiteResolverSubscriber skips all /api/* routes (expects Bearer token auth).
        // For this public endpoint we resolve the site from the Host header directly.
        if ($this->siteContext->hasSite()) {
            $site = $this->siteContext->getSite();
        } else {
            $site = $this->siteRepository->findByDomain($request->getHost());
            if ($site === null) {
                return $this->json([]);
            }
        }
        ...
        $articles = $this->em->getConnection()->fetchAllAssociative(
            'SELECT a.title, a.slug, c.slug AS cat_slug
             FROM articles a
             ...
             LIMIT :limit',
            ['siteId' => $siteId, 'status' => 'published', 'q' => $pattern, 'limit' => $limit],
        ...
        $terms = $this->em->getConnection()->fetchAllAssociative(
            'SELECT term, slug FROM glossary_terms
             ...
             LIMIT :limit',
            ['siteId' => $siteId, 'q' => $pattern, 'limit' => 4],
            ['limit' => ParameterType::INTEGER],
        );
